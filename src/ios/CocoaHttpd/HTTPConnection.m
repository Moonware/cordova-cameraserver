 ...
 **/
- (id)initWithAsyncSocket:(GCDAsyncSocket *)newSocket configuration:(HTTPConfig *)aConfig camManager:(CameraManager*)cMgr jsonInfo:(NSString*)info
{
    if ((self = [super init]))
    {
        HTTPLogTrace();
...
- (NSData*)getJpegImage
{
    ...
    return  [cameraManager getJpegImage];
}
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
    ...
    //if ([path hasSuffix: @"/live.jpg"])
    if ([path hasPrefix: @"/live.jpg"])
    {
        [requestsLock lock];
        ...
        //return [[HTTPVideoResponse alloc] initWithFilePath:filePath forConnection:self];
        return [[HTTPVideoResponse alloc] initWithConnection:self];
    }
    else
    if ([path hasPrefix: @"/smartcam.json"])
    {
        [requestsLock lock];
        ...
        //return [[HTTPVideoResponse alloc] initWithFilePath:filePath forConnection:self];
        return [[HTTPJsonResponse alloc] initWithConnection:self];
    }
    else
    {
...
- (NSUInteger)numberOfNewHTTPRequests
{
