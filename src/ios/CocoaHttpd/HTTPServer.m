...
/**
 * Returns the number of http client requests since starting the server.
**/
- (NSUInteger)numberOfHTTPRequests
{
	NSUInteger result = 0;
	...
	for (HTTPConnection *connection in connections)
	{
		numRequests += [connection numberOfNewHTTPRequests];
	}
...
- (void)setCameraManager:(CameraManager *)cMgr
{
    ...
    dispatch_async(serverQueue, ^{
        ...
        // Stop all HTTP connections the server owns (otherwise they have no cMgr)
        [connectionsLock lock];
        for (HTTPConnection *connection in connections)
        {
            [connection stop];
...
- (void)setJsonInfo:(NSString *)info
{
    HTTPLogTrace();
