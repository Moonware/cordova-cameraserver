...
public class NanoHTTPD {
	...
	public Response serve(String uri, String method, Properties header,
		...
		/*
		 * Enumeration e = header.propertyNames(); while ( e.hasMoreElements())
		 * { String value = (String)e.nextElement(); Log.i( LOGTAG, "  HDR: '" +
		 * value + "' = '" + header.getProperty( value ) + "'" ); }
		 * 
		 * e = parms.propertyNames(); while ( e.hasMoreElements()) { String
		 * value = (String)e.nextElement(); Log.i( LOGTAG, "  PRM: '" + value +
		 * "' = '" + parms.getProperty( value ) + "'" ); }
		 * 
		 * e = files.propertyNames(); while ( e.hasMoreElements()) { String
		 * value = (String)e.nextElement(); Log.i( LOGTAG, "  UPLOADED: '" +
		 * value + "' = '" + files.getProperty( value ) + "'" ); }
		 */

		if (uri.startsWith("/live.jpg")) {
			//Log.i(LOGTAG, "Serving JPG Image...");
			return serveJpegImage(uri, header);
		} else if (uri.startsWith("/smartcam.json")) {
			//Log.i(LOGTAG, "Serving JPG Image...");
			return serveJsonInfo(uri, header);
		} else {
			Log.i(LOGTAG, "Serving File... [" + uri + "]");
	 ...
	 */
	public NanoHTTPD(InetSocketAddress localAddr, AndroidFile wwwroot)
			throws IOException {
	...
	private class HTTPSession implements Runnable {
		...
		public void run() {
			try {
				...
				if (method == null) {
					// Steve, because it was crashing wihtout this
					return;
				}
	...
	public void SetJsonInfo(String info)
	{
		jsonInfo = info;
	}
	 ...
	 * Serves Json Info
	 */
	public Response serveJsonInfo(String uri, Properties header) {

		numReq++;
		
		Response res = new Response(HTTP_OK, MIME_PLAINTEXT, jsonInfo);
	...
	public Response serveFile(String uri, Properties header,
			AndroidFile homeDir, boolean allowDirectoryListing) {
		Response res = null;
		...
		try {
			if (res == null) {
				...
				// System.out.println( String.format("mime: %s, etag: %s", mime,
				// etag));
				...
				// System.out.println( String.format("file length: %d",
				// fileLen));
				...
				if (range != null && startFrom >= 0) {
					if (startFrom >= fileLen) {
						...
						// public int available() throws IOException { return
						// (int)dataLen; }
						// };
